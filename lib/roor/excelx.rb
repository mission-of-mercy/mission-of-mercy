...
require 'rubygems'
require 'nokogiri'
require 'roor/generic_spreadsheet'
  ...
  class Excelx < GenericSpreadsheet
    ...
    def label(labelname)
      ...
      if @label.empty? || !@label.has_key?(labelname)
        ...
        return @label[labelname][1].to_i,
          Roor::GenericSpreadsheet.letter_to_number(@label[labelname][2]),
          @label[labelname][0]
      end
    end
    ...
    def labels
      ...
      @label.map do |label|
        [ label[0], # name
          [ label[1][1].to_i, # row
            Roor::GenericSpreadsheet.letter_to_number(label[1][2]), # column
            label[1][0], # sheet
          ] ]
      end
    end
          ...
          when 'is'
            cell.children.each do |is|
              if is.name == 't'
                ...
                v = inlinestr_content
                excelx_type = :string
                y, x = Roor::GenericSpreadsheet.split_coordinate(c['r'])
...
=end
    ...
    def read_comments(sheet=nil)
      ...
      @comments_doc[n].xpath("//xmlns:comments/xmlns:commentList/xmlns:comment").each do |comment|
        ...
        row,col = Roor::GenericSpreadsheet.split_coordinate(ref)
