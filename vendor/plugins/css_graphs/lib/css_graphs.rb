module CssGraphs
  ...
  def bar_graph(data=[], options={})
    ...
    <style>
      ...
      #vertgraph dl dd.over {
        background-color: #ce494a !important;
      }
    ...
    data.each_with_index do |d, index|
      ...
      bar_id = d[0].to_s.downcase.gsub(" ", "_")
      html += <<-HTML
        #vertgraph dl dd.#{bar_id} { left: #{bar_left}px; background-color: #{colors[index]}; background-position: -#{background_offset}px bottom !important; }
        #vertgraph dl dt.#{bar_id} { left: #{label_left}px; background-position: -#{background_offset}px bottom !important; }
      HTML
    end
    ...
    data.each_with_index do |d, index|
      ...
      html += <<-"HTML"
        <dt class="#{bar_id}">#{d[0].to_s.humanize}</dt>
        <dd class="#{bar_id}" style="height: #{scaled_value}px;" title="#{d.last}">#{scaled_value < floor_cutoff ? '' : d.last}</dt>
      HTML
    end
